...
def _extract_blocks_from_single_block_statement_factory(
    ...
    def extract_blocks_from_single_block_statement(
        statement: ASTNode,
    ) -> List[BlockInfo]:
        return [
            BlockInfo(
...
def _extract_blocks_from_switch_branching(statement: ASTNode) -> List[BlockInfo]:
    return [
        BlockInfo(
            reason=BlockReason.SINGLE_BLOCK,
            statements=[
                switch_statement for switch_case in statement.cases for switch_statement in switch_case.statements
            ],
        )
    ]
...
def _extract_blocks_from_try_statement(statement: ASTNode) -> List[BlockInfo]:
    ...
    if statement.resources is not None:
        block_infos.append(BlockInfo(reason=BlockReason.TRY_RESOURCES, statements=statement.resources))
...
def _unwrap_block_to_statements_list(block_statement_or_statement_list: Union[ASTNode, List[ASTNode]]) -> List[ASTNode]:
